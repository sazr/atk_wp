...
class App extends \Atk4\Ui\App {
  ...
  public static function get_app() {
    ...
    add_action( 'wp_ajax_' . self::NAME,       [$this, 'ajax_callback']);
    add_action( 'wp_ajax_nopriv' . self::NAME, [$this, 'ajax_callback']);
	}
  ...
  public function url($page = [], $needRequestUri = false, $extraRequestUriArgs = []) {
    ...
    if (isset($page['atk_wp_wizard'])) {
      error_log('dffd');
      error_log(print_r($extraRequestUriArgs, true));
      if (is_admin() && isset($_GET['page'])) {
        $extraRequestUriArgs['page'] = $_GET['page'];
        // $this->page = null;
      }
    }
    else $this->page = admin_url('admin-ajax');
